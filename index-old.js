...
//------------------------------------------------------------------------------
//Public Constructor
function SonyCamera(url, port, path) {
...
//inherit funcitons from EventEmitter
util.inherits(SonyCamera, EventEmitter);

//------------------------------------------------------------------------------
//Public Functions
SonyCamera.prototype.show = function () {
  //logs the conection information to the console
  console.log(this.url + ':' + this.port + this.path);
};

//Function to send command to the camera
...
//calls getEvent and processes the result
      ...
      if(semver.gte(version, minVersionRequired)) {
        var connected = function() {
          ...
          var _checkEvents = function(err) {
            if(!err) {
              if(self.connected) setTimeout(function(){self._processEvents(false, _checkEvents)}, 5000); else console.log("SonyWifi: disconnected, stopping event poll");
            } else {
...
SonyCamera.prototype.setShootingMode = function (callback) {
  ...
  if(this.downloading) {
    console.log("SonyWifi: camera not ready.  Downloading File");
    return callback && callback('downloading');
  }
  ...
  console.log(this.cameraFunction);
};
...
SonyCamera.prototype.setTransferMode = function (callback) {
  ...
  if(this.status != "IDLE") {
    console.log("SonyWifi: camera is busy.  Status:", this.status);
    return callback && callback('camera not ready');
  }
...
SonyCamera.prototype.getLastFile = function (id, filePath, remove, callback) {
  var self = this;
  ...
  var processResult = function(err, output) {
    ...
    downloadFile = function(path){
      var file = fs.createWriteStream(filePath + id +".MP4");
      http.get(path, function(response) {response.pipe(file);});
    }
